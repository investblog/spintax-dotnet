...
# --- C# / .NET (see .agents/rules/quality-csharp.md) ---
# The product of this repository IS the dll, so a missing SDK BLOCKS instead of skipping: a
# gate that passes with nothing to build is the "green while checking nothing" failure that
# docs/TODO.md recorded before the first .cs existed.
if tracked '*.cs' '*.csproj' '*.sln'; then
  if have dotnet; then
    # Warnings are errors (Directory.Build.props), so a clean build is the lint.
    run dotnet build Spintax.sln -nologo -v q
    if [ "$mode" = push ]; then
      run dotnet format Spintax.sln --verify-no-changes -v q
      run dotnet test Spintax.sln --no-build -nologo -v q
      # The corpus is the acceptance suite and it is NOT skip-if-missing: the runner exits 2
      # when no corpus resolves (ADR-0001), so forgetting SPINTAX_FIXTURES blocks the push.
      # --baseline makes it a gate while the port is red: a NEW failure is a regression and a
      # case that starts passing must be removed from tests/known-failures.txt.
      # Two hosts: net8.0 is the everyday one, net472 is the runtime class ZennoPoster loads
      # the dll into — the BCL's Unicode tables are the one thing that can differ between them.
      run dotnet run --project tests/Spintax.Corpus -f net8.0 --no-build -- --baseline tests/known-failures.txt
      run dotnet run --project tests/Spintax.Corpus -f net472 --no-build -- --baseline tests/known-failures.txt
    fi
  else
    echo "git-quality-gate: BLOCK - dotnet SDK not found, and this repository's product is a .NET dll" >&2
    fail=1
  fi
fi
