...
- **Claude** → merge `claude.json` into `./.claude/settings.json` (`hooks.PreToolUse`) — the
  ...
  [CRITICAL] The `matcher` must name **every** shell tool the session exposes, not just `Bash`:
  a matcher is a regex over tool NAMES, so a tool it omits runs unguarded. On native Windows the
  primary shell tool is `PowerShell` — omitting it leaves `Get-Content .env` unblocked while
  `cat .env` is caught. Check the session's actual tool names when a new one appears.
